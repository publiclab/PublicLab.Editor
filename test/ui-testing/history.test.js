...
describe(
  ...
  () => {
    test("has log, key, interval and id", async () => {
      ...
      expect(
        await page.evaluate(() => editor.history.options.interval)
      ).toBeDefined();
      expect(
        await page.evaluate(() => editor.history.options.id)
      ).toBeDefined();
    });
    ...
    test("adds, fetches and stores", async () => {
      ...
      expect(
        await page.evaluate(
          () => document.querySelector(".ple-history-saving").style.display
        )
      ).toBe("none");
      await page.evaluate(() => editor.history.add("some text"));
      expect(await page.evaluate(() => editor.history.last().text)).toBe(
        "some text"
      );
      expect(
        await page.evaluate(() => editor.history.last().timestamp)
      ).toBeDefined();

      expect(await page.evaluate(() => editor.history.fetch().length)).toEqual(
        1
      );
      expect(await page.evaluate(() => editor.history.last().text)).toBe(
        "some text"
    ...
    test("creates new log entry when value() set", async () => {
      expect(
        await page.evaluate(
          () => editor.richTextModule.options.textarea.value.length
        )
      ).toBeGreaterThan(0);
      expect(
        await page.evaluate(() => editor.richTextModule.options.textarea)
      ).toBeDefined();
      await page.evaluate(() =>
    ...
    test("stores only 20 items until we optimize it", async () => {
      ...
      expect(await page.evaluate(() => editor.history.log.length)).toBe(20);
      expect(await page.evaluate(() => editor.history.log[0].text)).toBe(
        "some text 10"
      );
    });
    ...
    test("writes out history to a DOM element", async () => {
      ...
      for (let i = 0; i < 10; i++) {
        ...
        expect(await page.evaluate(() => $("#history p.log").length)).toBe(
          1 + i
        );
      }
      expect(await page.evaluate(() => $("#history p.day").length)).toBe(1);
