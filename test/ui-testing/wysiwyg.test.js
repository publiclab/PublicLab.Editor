...
describe('Wysiwyg', () => {
  ...
  test("runs post-conversion filter", async () => {
    try {
      await page.evaluate(() => {
        // this next line is commented out because it seems to make this test timeout. However, it doesn't stop the test from passing
        // because regardless of the wysiwyg.mode, wysiwyg.value() will always be the same, so the expect block always pass.
