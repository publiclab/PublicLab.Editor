...
describe("Errors", () => {
  test("Editor exists and has a textarea", async () => {
    expect(
      await page.evaluate(() => document.querySelector(".ple-textarea"))
    ).toBeDefined();
    expect(await page.evaluateHandle(() => editor)).toBeDefined();
    expect(
      await page.evaluateHandle(() => editor.options.textarea)
    ).toBeDefined();
    expect(
      await page.evaluateHandle(() => editor.options.textarea.innerHTML)
    ).toEqual(
      await page.evaluateHandle(
        () => document.querySelector(".ple-textarea").innerHTML
      )
    );
  });
  ...
  test("counts valid modules and enables publish button", async () => {
    expect(
      await page.evaluate(() => editor.titleModule.el.find("input")[0].value)
    ).toBe("");
    ...
    ); // set editor mode to markdown
    await page.evaluate(() => editor.richTextModule.value("")); // empty editor content on
    expect(await page.evaluate(() => editor.richTextModule.valid())).toBe(
      false
    );
